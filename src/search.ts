...
export async function qmdSearch(
  ...
  n: number,
  config: LucidConfig
): Promise<QmdRawResult[]> {
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [config.qmdShimPath, "search", prompt, "-n", String(n), "--json"],
    ...
    child.on("close", (code) => {
      ...
      try {
        ...
        const parsed = JSON.parse(stdout.slice(start, end + 1)) as QmdRawResult[];
...
function getRecencyMultiplier(filePath: string): number {
  ...
  for (const { maxDays, weight } of RECENCY_WEIGHTS) {
    if (daysDiff <= maxDays) return weight;
  }
  return 0.8;
}

function getTypeWeight(filePath: string): number {
...
function getCollectionWeight(filePath: string): number {
...
export function scoreBySalience(results: QmdRawResult[]): ScoredResult[] {
  return results
    .map((r) => ({
      file: r.file,
...
export function formatResultsAsContext(results: ScoredResult[]): string {
  ...
  for (const r of results) {
    ...
    const clean = r.snippet
      ...
      .trim();
    if (r.title && r.title !== r.file) lines.push(`*${r.title}*`);
